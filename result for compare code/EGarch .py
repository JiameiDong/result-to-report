...
present= pd.DataFrame({"ret":table['ret'][pt:],
                        "predict":yhat1[pt:],
                        "vol":table['vol'][pt:],
                        },index=table.index[pt:])
predict_vol = pd.DataFrame({
                    "predict":present["predict"][-TEST_EXAMPLES:]},
                    index=present.index[-TEST_EXAMPLES:]
                    )                        
present=present[present.predict>0]
present=present[present.predict<4]
predict_vol=predict_vol[predict_vol.predict>0]
predict_vol=predict_vol[predict_vol.predict<4]
plt.plot(predict_vol,label="predict")
plt.plot(present["vol"],label="vol")




yhat1=predict_vol["predict"]
test_vol1=present['vol']
test_vol=test_vol1[-len(yhat1):]
