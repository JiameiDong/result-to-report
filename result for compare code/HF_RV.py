...
data_HF = pd.read_csv("D:/RA/result for teacher\CSI300_15min.csv",
                    sep=',',
                    encoding = "utf-8",
                    parse_dates={'datetime':['TDATE1','MINTIME']},
                    date_parser=dateparse)    #读取数据

table_HF = pd.pivot_table(data_HF,index=['datetime'],values=['ENDPRC'])  #日期、价格放同一个表中
...
table_HF['r2']=ret_HF*ret_HF

#get daily data
dateparse = lambda dates:pd.datetime.strptime(dates,'%Y%m%d')  #读取日期格式

data_daily = pd.read_csv("D:/RA/result for teacher/CSI300_daily.csv",
                    sep=',',
                    encoding = "utf-8",
                    parse_dates=['TDATE'],
                    date_parser=dateparse)    #读取数据

table_daily = pd.pivot_table(data_daily,index=['TDATE'],values=['ENDPRC'])  #日期、价格放同一个表中
...
for i in range(1,len(price_daily)):
        ret_daily[i] = (math.log(price_daily[i])-math.log(price_daily[i-1])) * 100
